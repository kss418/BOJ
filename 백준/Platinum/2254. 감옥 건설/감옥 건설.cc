...
public:
    ...
    void add(pll a, ll cnt){ arr.push_back({a, cnt}); }
...
int main() {
    ...
    ll cx, cy; cin >> n >> cx >> cy;
    for(int i = 1;i <= n;i++) cin >> a[i].x >> a[i].y;

    ll result = 0; _inter inter;
    while(1){
        _ch ch; ll cnt = 0;
        for(int i = 1;i <= n;i++) {
            if(use[i]) continue;
        ...
        vector <ll> ret = ch.num;
        for(auto& i : ret) use[ch.arr[i].idx] = 1;
        vector <pll> p = ch.ret();

        if(inter.ret(p, {cx, cy})) result++;
        else break;
    }
