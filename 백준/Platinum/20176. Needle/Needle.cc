...
constexpr ll MAX = 60100; // SET MAX SIZE
constexpr ll MOD = 998244353;
constexpr ll MID = 30030;
vector <ll> a(MAX), b, c(MAX);
ll result;
...
void run(){
    cin >> n;
    for(int i = 1;i <= n;i++) cin >> m, a[m + MID] = 1;

    cin >> n;
    for(int i = 1;i <= n;i++) cin >> m, b.push_back(m + MID);

    cin >> n;
    for(int i = 1;i <= n;i++) cin >> m, c[m + MID] = 1;

    vector <ll> ret = fft.mul(a, c);
    for(auto& i : b) result += ret[2 * i];
