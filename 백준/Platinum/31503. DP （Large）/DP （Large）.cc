...
void run(){
    cin >> n >> m; _seg lis(n), lds(n);
    for(int i = 1;i <= n;i++) cin >> a[i], vec.push_back(a[i]);

    sort(all(vec));
    vec.erase(unique(all(vec)), vec.end());
    for(int i = 0;i < vec.size();i++) num[vec[i]] = i + 1;
    for(int i = 1;i <= n;i++) a[i] = num[a[i]];

    for(int i = 1;i <= n;i++){
        ll q = max(lis.query(1, a[i] - 1) + 1, lis.query(a[i], a[i]));
        lis.update(a[i], q);
        result[i] += q;
    }

    for(int i = n;i >= 1;i--){
        ll q = max(lds.query(a[i] + 1, n) + 1, lds.query(a[i], a[i]));
        result[i] += lds.query(a[i] + 1, n);
        lds.update(a[i], q);
    }
