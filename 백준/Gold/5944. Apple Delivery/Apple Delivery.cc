...
void run(){
    ...
    dij.init(st);
    ll now = dij.ret(en[0]); dij.init(en[0]);
    result = min(result, now + dij.ret(en[1]));

    dij.init(st);
    now = dij.ret(en[1]); dij.init(en[1]);
    result = min(result, now + dij.ret(en[0]));
