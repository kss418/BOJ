...
constexpr ll MAX = 250101; // SET MAX SIZE
constexpr ll MOD = 998244353;
ll sz[MAX], result, p[MAX];
vector <ll> adj[MAX], c;
bitset <251010> dp;
...
void cent(ll cur = 1, ll pre = -1){
    ...
    if(!flag && 2 * (n - sz[cur]) <= n) c.push_back(cur);
}
...
void run(){
    ...
    for(auto& cur : c){
        ...
        for(auto& nxt : adj[cur]){
            arr.push_back(nxt == p[cur] ? n - sz[cur] : sz[nxt]);
        }
        sort(all(arr));
